    ...
    def ellipseFromCoordsClassifier(self, contours, RGBimg, classifyNet, classList, frameNumber):
        ...
        for contour in contours:
            if len(contour) >= 5:
                ...
                testsegment = np.array(blackBackSegment)  ## segment that is saaved to file for test analysis purposes
                segment = blackBackSegment

                with torch.no_grad():
                    ## convert to proper shape, type
                    segment = np.transpose(segment, [2, 0, 1])
                    segment = np.expand_dims(segment, 0) ## batch size of 1 input
                    ...
                    # classPrediction = classPrediction.cpu().numpy() ## convert to numpy array
                    _, index = torch.max(classPrediction, dim=1) ## getting largest value for prediction output

                    classPrediction = classPrediction.cpu().numpy()
