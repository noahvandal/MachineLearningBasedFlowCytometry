...
class Dataset(Dataset):
    '''
    Dataloader for the images and masks dataset. 

    Structure must be as follows:
    rootDir
        - Images
        - Masks
    with images and corresponding masks having the same name.
    Will be loaded as pytorch tensor types. 
    ...
    def __getitem__(self, index):
        ...
        ## return corresponding name if is a validation image
        if self.eval:
...
# getting datset using dataload function and native functions from torch; treated more like a generator
...
def rgbToOnehotBW(rgb, colorDict):
    ...
    for label, color in enumerate(colorDict.keys()):
        ## wherever cell region is, make black; else white. 
...
## instead of usign one-hot encoding, use sparse encoding. Not implemented for our training
...
def outputClassImages(onehot, color_dict):
    '''
    for dictionary of different class values, given a one-hot input (type of output of network and also truth mask), will output rgb image with correct label color scheme. 
    '''
...
def RGBtoBW(image, isGrayscale=False):
    '''
    This function was used when trying to perform inference using the MC model, but with binary output and classification. Not used in final result.
    '''
...
def onehotToIMG(img, bw=False):
    '''
    Another method to convert a one-hot encoded image to RGB.
    '''
    img = np.array(img)
